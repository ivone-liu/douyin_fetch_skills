...
Use the KB manifest to query the Haystack + Qdrant store first. Retrieve the most relevant chunks for the user topic, then generate from retrieved evidence instead of from memory.
...
- 3-option idea set with one recommended option when the user asks for variants
...
Do not read only static pattern JSON if a Qdrant-backed KB exists. Query the KB through Haystack first.
