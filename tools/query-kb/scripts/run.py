...
def answer_with_llm(manifest: dict, question: str, top_k: int) -> dict:
    ...
    payload = call_llm_json(
        system_prompt='你是一个抖音拍摄知识库问答助手。必须只基于检索到的上下文回答。输出 JSON，字段包含 answer、evidence_video_ids、actionable_points、uncertainty。',
...
def main() -> int:
    ...
    manifest = load_manifest(kb_dir / 'knowledge-base.json')
    cfg = RagConfig()
    result = answer_with_llm(manifest, args.query, args.top_k) if cfg.llm_available() else answer_without_llm(manifest, args.query, args.top_k)
