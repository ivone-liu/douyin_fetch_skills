...
- Python >= 3.10
- 推荐 Python 3.10 或 3.11，不再兼容 Python 3.9
...
说明：如果你的 OpenClaw/OpenAI-compatible 网关在本机且不要求鉴权，可以不填 `OPENCLAW_API_KEY` / `OPENAI_API_KEY`。v3.3 已支持“有 base URL + model 但无 API key”的本地无鉴权网关。
...
```bash
QDRANT_MODE=local bash install.sh --skip-system-deps
```
